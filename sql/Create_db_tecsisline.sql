...
values (
        ...
        now(),
        now()
    ), (2, "empleado", now(), now()), (3, "cliente", now(), now());
...
values (1, "Masculino", now(), now()), (2, "Femenino", now(), now()), (3, "Otro", now(), now()), (4, "Indefinido", now(), now());
...
alter table
    direcciones change direccion_id direccion_id int auto_increment;
...
alter table usuarios
add
    constraint fk_usuario_tipo_documento foreign key(tipo_documento_id) references tipo_documentos(tipo_documento_id);
...
alter table usuarios
add
    constraint fk_usuario_sexo foreign key (sexo_id) references sexos(sexo_id);

alter table factura
add
    constraint fk_factuara_usuario foreign key (
        no_documento,
        tipo_documendo_id
    ) references usuarios(
        no_documento,
...
alter table factura
add
    constraint fk_factuara_TipoPago foreign key (tipo_pago_id) references tipo_pago(tipo_pago_id);
...
alter table productos
add
    constraint fk_producto_factura foreign key (factura_id, tipo_pago_id) references factura(factura_id, tipo_pago_id);

alter table marca_producto
add
    constraint fk_MarcaProducto_marca foreign key(marca_id) references marcas(marca_id);

alter table marca_producto
add
    constraint fk_MarcaProducto_producto foreign key (
        producto_id,
        factura_id,
        tipo_pago_id
...
add
    constraint fk_direccion_usuario foreign key (no_documento) references usuarios (no_documento);

alter table efectivos
add
    constraint fk_Efectico_TipoPago foreign key (tipo_pago_id) references tipo_pago (tipo_pago_id);

alter table tarjeta_credito
add
    constraint fk_TarjetaCredito_TipoPago foreign key (tipo_pago_id) references tipo_pago (tipo_pago_id);
